...
export const OfferCard: React.FC<OfferCardProps> = ({ offer, onClaimClick, onMoreInfo }) => {
  ...
  return (
    <div id={`offer-card-${offer.id}`} className="offer-card retro-window group relative flex h-full min-w-0 flex-col overflow-hidden rounded-[1.35rem] border border-white/[0.1] bg-[#171a22]/95 shadow-[0_18px_50px_rgba(0,0,0,0.2),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-cyan-200/30 hover:shadow-[0_24px_60px_rgba(0,0,0,0.32),0_0_30px_rgba(45,212,238,0.1)]">
      <div className="relative flex h-32 shrink-0 items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_50%_0%,rgba(255,255,255,0.16),transparent_34%),radial-gradient(circle_at_20%_100%,rgba(45,212,238,0.2),transparent_42%),linear-gradient(145deg,#263747,#0d141f_78%)] sm:h-36">
        ...
        <a href={claimUrl} onClick={handleOfferLinkClick} onPointerDown={(event) => event.stopPropagation()} aria-label={`Open ${offer.title} offer from ${offer.company}`} className="relative z-10 flex h-full w-full items-center justify-center transition-transform duration-300 hover:scale-[1.04] focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#2dd4ee]/70">
      ...
      <div className="flex flex-1 flex-col bg-[linear-gradient(180deg,#1b1e27_0%,#171a22_100%)] p-3 sm:p-4">
        <div className="flex items-start gap-2">
          <a href={claimUrl} onClick={handleOfferLinkClick} onPointerDown={(event) => event.stopPropagation()} aria-label={`Open ${offer.title} offer from ${offer.company}`} className="min-w-0 flex-1 text-left transition-opacity hover:opacity-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#2dd4ee]/70 rounded-lg">
          ...
          <button type="button" onClick={() => onMoreInfo(offer)} onPointerDown={(event) => event.stopPropagation()} aria-label={`More information about ${offer.company}`} className="focus-ring inline-flex shrink-0 items-center gap-1 rounded-full border border-cyan-200/20 bg-cyan-200/[0.06] px-2.5 py-2 text-[10px] font-semibold text-cyan-100 transition-colors hover:bg-cyan-200/15 hover:text-white">

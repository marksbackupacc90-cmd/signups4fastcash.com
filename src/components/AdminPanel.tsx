...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [adminPageOpen, setAdminPageOpen] = useState(true);
      ...
      {adminPageOpen && activeAdminTab === 'records' && visitorAnalytics && (
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <div className="sm:col-span-3 rounded-xl border border-white/[0.08] bg-[#0e121a] p-4">
            <button
              type="button"
              onClick={() => setExpandedLocations((current) => !current)}
              aria-expanded={expandedLocations}
              className="flex w-full flex-wrap items-center justify-between gap-2 text-left"
            >
              <div>
                <div className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-wider text-zinc-400">
                  <ChevronDown className={`h-3.5 w-3.5 transition-transform ${expandedLocations ? 'rotate-180' : ''}`} />
                  Approximate visitor locations
                </div>
            ...
            {expandedLocations && visitorAnalytics.locations.length > 0 ? (
              <div className="mt-3 overflow-x-auto">
